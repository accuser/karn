...
2. **Pre-flights** — checks that `bynkc`, Node, and `wrangler` are usable, with
   ...
   doctor`](../editor-and-tooling/doctor.md). A missing tool fails here, before
   anything is built.
...
--only deploy`](../editor-and-tooling/doctor.md) to see exactly what you have.
...
Cloudflare Workers](cloudflare-workers.md) for the manual `wrangler deploy`
flow.

## Related

- [Target Cloudflare Workers](cloudflare-workers.md) — the two emission targets
  and the manual recipe `bynk dev` runs for you.
- [Check your environment with `bynk doctor`](../editor-and-tooling/doctor.md) —
