...
the token-and-trivia summary — is the authoritative definition of Karn's syntax.
It is generated from the `tree-sitter-karn` grammar, so it cannot drift from the
