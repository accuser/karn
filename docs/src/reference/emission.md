...
Karn compiles to TypeScript. This page summarises what each construct emits. The
output is deterministic and headed `// Generated by karnc — do not edit by hand.`
...
See [Target Cloudflare Workers](../guides/projects-build-and-deployment/cloudflare-workers.md).

```mermaid
flowchart TD
  src["Karn source"] --> cm["commons → plain TS types and functions"]
  src --> b["target: bundle"]
...
A per-target test module plus an aggregating `tests/main.ts` runner; `karnc test`
