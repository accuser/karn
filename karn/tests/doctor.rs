//! `karn doctor` — the capability/exit-code matrix, the detection probe, and
...
//! host's tools. The goldens are blessed with `KARN_BLESS=1`.

use std::collections::HashMap;
use std::path::{Path, PathBuf};

use karn::compiler::{Compiler, Origin, Skew};
use karn::doctor::{self, Capability, Context, DoctorOptions, Level};
use karn::probe::{self, DetectOpts, Provenance, Toolbox, Version};
...
/// A `karnc` resolved on PATH, with the given skew vs the driver.
fn karnc_ok(skew: Skew) -> Compiler {
    Compiler {
        path: Some(PathBuf::from("/opt/karn/bin/karnc")),
...
fn bare_compile_only_env_exits_zero_with_test_dev_flagged() {
    // Only karnc; no node/tsc/tsx/wrangler.
    let fake = Fake::default();
    let report = doctor::diagnose(&fake, &karnc_ok(Skew::Match), &ctx(false), &bare());
...
fn everything_present_is_all_green_exit_zero() {
    let fake = Fake::default()
        ...
        .path_tool("karnc-lsp", "/usr/bin/karnc-lsp", Some(v(9, 9, 9)));
    let report = doctor::diagnose(&fake, &karnc_ok(Skew::Match), &ctx(false), &bare());
...
fn broken_compile_floor_fails_even_bare() {
    let fake = Fake::default()
        .path_tool("node", "/usr/bin/node", Some(v(20, 0, 0)))
        .path_tool("tsc", "/usr/bin/tsc", Some(v(5, 4, 2)));
    let report = doctor::diagnose(&fake, &karnc_missing(), &ctx(false), &bare());
...
fn minor_skew_warns_major_skew_fails() {
    ...
    let minor = doctor::diagnose(&fake, &karnc_ok(Skew::Minor), &ctx(false), &bare());
    ...
    let major = doctor::diagnose(&fake, &karnc_ok(Skew::Major), &ctx(false), &bare());
...
fn only_filter_scopes_probes() {
    ...
    let report = doctor::diagnose(&fake, &karnc_ok(Skew::Match), &ctx(false), &opts);
...
fn golden_report() -> doctor::Report {
    ...
    let mut report = doctor::diagnose(&fake, &karnc_ok(Skew::Match), &ctx(false), &bare());
...
fn bless_or_assert(name: &str, actual: &str) {
    ...
    let expected = std::fs::read_to_string(&path).unwrap_or_else(|_| {
        panic!(
            "missing golden {}; regenerate with KARN_BLESS=1 cargo test -p karn",
            path.display()
        )
    });
    assert_eq!(
        actual, expected,
        "golden {name} drifted; re-bless with KARN_BLESS=1 cargo test -p karn"
    );
}
...
fn human_smoke() {
    ...
    assert!(out.contains("karn doctor"));
}
