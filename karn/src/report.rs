...
//! scriptable surface** (golden-tested), siblings to `karnc check --format
//! short` (ADR 0071).
...
fn human(report: &Report) -> String {
    ...
    let header = if report.is_all_ok() {
        "karn doctor — your environment is ready".to_string()
    } else {
        "karn doctor — environment report".to_string()
    };
    out.push_str(&header);
    out.push('\n');
    out.push_str(&format!("driver: karn {}\n", report.driver_version));
    if let Some(path) = report.compiler.path.as_deref() {
        out.push_str(&format!("compiler: karnc at {}\n", path.display()));
    }
    out.push('\n');
