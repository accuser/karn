//! The `karn` driver command-line interface.
//!
//! Kept thin: the driver shells `karnc` and the Node toolchain. v0.46 ships a
...
#[derive(Parser, Debug)]
#[command(name = "karn", version, about = "The Karn driver", long_about = None)]
pub struct Cli {
    #[command(subcommand)]
    pub command: Command,
}
...
pub enum Command {
    /// Check whether your machine is ready to compile, test, and deploy Karn —
    /// and print the exact remedy for anything missing.
    ///
    /// Bare `karn doctor` is informational: it surveys every capability and
    /// exits 0 unless `karnc` itself is unusable. `--only <capability>` gates on
...
pub enum CapabilityArg {
    /// `karnc` compile / check / fmt.
    Compile,
    /// `karn test` — Node + tsc|tsx.
    Test,
    /// dev / deploy to Cloudflare — Node + wrangler.
    Deploy,
    /// Editor support — karnc-lsp.
    Editor,
    /// Build Karn from source — a Rust toolchain.
    Build,
}
