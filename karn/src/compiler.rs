//! Locate the `karnc` compiler the driver shells, and report
//! **driver↔compiler version skew**.
//!
//! Resolution order (ADR: introduce the `karn` driver):
//!
//! 1. an explicit override — the `KARN_KARNC` environment variable (the
//!    `karn.executablePath`-style escape hatch);
//! 2. `karnc` on `PATH`;
//! 3. a `karnc` sibling of the running `karn` binary (mirrors how `vscode-karn`
//!    resolves `karnc-lsp` next to itself).
//!
...
//! resolution can pick a `karnc` whose version differs from the driver's: once
//! they are separate binaries, a global `karn 0.46` can shell a stale `karnc
...
/// How `karnc` was located.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Origin {
    /// From the `KARN_KARNC` override.
    Override,
    /// From the global `PATH`.
    Path,
    /// A sibling of the running `karn` binary.
    Sibling,
}
...
/// A resolved (or unresolved) `karnc`.
#[derive(Debug, Clone)]
pub struct Compiler {
    /// `None` when `karnc` could not be located at all — the broken compile
...
/// Resolve `karnc` against a [`Toolbox`], given the override (typically
/// `std::env::var("KARN_KARNC")`), the directory of the running `karn` binary
/// (for the sibling fallback), and the driver's own version (to classify skew).
pub fn resolve(
    tb: &dyn Toolbox,
    override_path: Option<&Path>,
    karn_bin_dir: Option<&Path>,
    driver: Version,
) -> Compiler {
    let (path, origin) = locate(tb, override_path, karn_bin_dir);
...
fn locate(
    ...
    if let Some(p) = tb.on_path("karnc") {
        return (Some(p), Some(Origin::Path));
    }
    if let Some(dir) = karn_bin_dir
        && let Some(p) = tb.in_dir(dir, "karnc")
    {
...
fn file_stem(p: &Path) -> &str {
    p.file_stem().and_then(|s| s.to_str()).unwrap_or("karnc")
}
