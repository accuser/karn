//! `karn` — the Karn driver.
//!
//! A thin orchestrator over the [`karnc`] compiler and the Node toolchain:
//! `karn` is to `karnc` what `cargo` is to `rustc`. The compiler stays pure
//! (compile / check / fmt / test); environment orchestration — "is `wrangler`
//! installed", "is your machine ready" — lives here (ADR: introduce the `karn`
//! driver).
//!
...
//! - [`compiler`] — locate `karnc` (override → PATH → sibling-of-`karn`) and
...
/// `karnc`'s version to detect skew ([`compiler::Skew`]).
