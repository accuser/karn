...
fn emit_integration_module(
    ...
    // Forward slashes so the map's `sources` are portable across platforms.
    let mut module_smb = SourceMapBuilder::new();
    let src_id = module_smb.add_source(rel_path.replace('\\', "/"), source.to_string());
...
fn emit_test_module(
    ...
    for &i in indices {
        ...
        for case in &test_decl.cases {
            ...
            let src_id =
                module_smb.add_source(rel_path.replace('\\', "/"), parsed[i].source.clone());
