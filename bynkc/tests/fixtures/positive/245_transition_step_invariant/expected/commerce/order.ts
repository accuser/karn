...
export type AuthId = string & { readonly __brand: "commerce.order.AuthId" };
  ...
  private async commitState(s: OrderState): Promise<void> {
    ...
    if (__prior !== undefined) {
      ...
      if (!((!(__old.status.tag === "Paid") || __new.status.tag === "Paid"))) {
        console.error("InvariantViolation Order.paid_is_terminal", { agent: "Order", invariant: "paid_is_terminal" });
