//! Golden-output tests for the Karn formatter.
//!
...
//!
//!     KARN_BLESS=1 cargo test -p karn-fmt --test golden
//!
...
fn golden_fixtures_format_canonically() {
    ...
    assert!(
        !dirs.is_empty(),
        "no golden fixtures found under karn-fmt/tests/fixtures/"
    );
            ...
            Err(e) => {
                failures.push(format!(
                    "{name}: cannot read expected.karn ({e}) — \
                     regenerate with KARN_BLESS=1 cargo test -p karn-fmt --test golden"
                ));
                continue;
            }
        };
        ...
        if got != expected {
            failures.push(format!(
                "{name}: format(input) != expected\n{}\n  \
                 (regenerate with KARN_BLESS=1 if this change is intended)",
