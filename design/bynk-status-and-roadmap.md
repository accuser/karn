...
| **Driver `bynk`** (v0.46–v0.57) | Growing | Thin orchestrator over `bynkc` (override → PATH → sibling resolution). `bynk doctor` — environment check with a pinned output/exit contract (ADRs 0083–0084). `bynk dev` (v0.57) — build + serve locally via `wrangler dev` in local mode (compile-once MVP). Next intent: the `dev` watch loop, multi-worker local dev, `new` scaffolding, and `deploy` (provisioning). |
