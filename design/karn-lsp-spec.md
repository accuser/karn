...
**Coverage.** Top-level named declarations: types, free `fn`s, capabilities, services, agents, providers. **Instance methods (v0.36, ADR 0069)** are also indexed, keyed by the compound name `"Type.method"` — the def is the method-name span, and a method **call** (`x.m()`) is a reference, recorded already-spelled from the receiver type the checker resolved and qualified through the same `uses`/`consumes` walk as a cross-file type reference (so a same-named method on two types stays distinct: `Counter.bump` ≠ `Gauge.bump`). Reference sites include every way such a symbol is named — annotation and static-receiver type positions (`T.of`, `T.Variant`, `T { … }`, pattern qualifiers, `Mock[T]`), fn calls and first-class fn values, `given` clauses (bare, dotted `B.Cap`, flattened), capability op-call receivers, cross-context service calls, the clause lists (`exports opaque/transparent { T }`, `exports capability { Cap }`, `consumes U { Cap }` selections), and references from **test and integration units** (including a test body's `svc.call(…)`). Spans cover the **name segment only** — in `shop.billing.Pay` the reference is `Pay`; in `Counter.bump` the method site is `bump`.
...
**Deferred kinds** (no entries yet): match-arm / `is`-narrowing bindings, record fields, capability op names (instance methods are now indexed — see above). First-party `karn.*` units are excluded — they are not user-editable.
...
`textDocument/rename` renames a symbol project-wide; `textDocument/prepareRename` (declared via `prepareProvider: true`) validates the position first and **refuses** (returns null) on anything the index does not cover — locals, fields, op names, and unit/context names (renaming a unit implies a file move; that is the A-3 file-operations increment). Methods rename (v0.36): the edit targets the member segment of the compound `"Type.method"` key, never the `Type.` prefix.
...
`textDocument/implementation` on a **capability** symbol — its `capability Cap` declaration, a `given Cap` use, or a `provides Cap` use — returns the definition site(s) of every **provider** that implements it (the Karn analogue of "go to implementations" on an interface). Served from the binding index's **implementation graph**: an `ImplEdge { capability, provider, site }` side table on `ProjectIndex`, built like the v0.34 call graph by resolving a reference's `owner` through `owner_keys`.
