...
- **Spec:** `design/karn-lsp-spec.md` §3.14
...
`vscode-karn` declared none, so v0.28's Karn-distinctive tokens were
...
test**: a `karn-lsp` test parses `vscode-karn/package.json` and asserts
...
the crate, so it is **`exclude`d from the published `karn-lsp` tarball**
