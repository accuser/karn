...
`1.` and `.5` are `bynk.parse.malformed_float_literal`. `1` stays
...
- **Reject overflow:** a literal that parses to a non-finite double
  (`1e999`) is `bynk.lex.float_literal_overflow`, mirroring `IntLit`'s
  out-of-range reject. (The proposal sketched this code under
  `bynk.parse.*`; it lands in `bynk.lex.*` because the check runs in
  `tokenize()` exactly where `bynk.lex.integer_overflow` does.)
