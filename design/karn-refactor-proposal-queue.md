# Karn compiler — refactor proposal queue

The concrete backlog of *internal-quality* refactors for the `karnc` crate, ordered by recommended
sequence. A planning reference (complements `karn-tooling-proposal-queue.md`, which tracks *feature*
...
These items were captured from a code-quality review of the `karnc` binary (June 2026). The review's
...
5. **Collapse the `compile_project*` API into an options struct.** Six public variants (`_full`,
   ...
   re-exports and every caller (`main.rs`, `karn-lsp`).*
