//! `mdbook-karn-visuals` — an mdBook preprocessor for diagrams and callouts.
//!
...
//!     `theme/karn-callouts.css`. Blank lines around the body keep its inner
...
//!   * `mdbook-karn-visuals supports <renderer>` → exit 0 iff supported.
...
fn main() {
    ...
    if std::io::stdin().read_to_string(&mut input).is_err() {
        eprintln!("mdbook-karn-visuals: failed to read stdin");
        exit(1);
    }
    let mut parsed: Value = match serde_json::from_str(&input) {
        Ok(v) => v,
        Err(e) => {
            eprintln!("mdbook-karn-visuals: invalid preprocessor input: {e}");
            exit(1);
        }
    };
    ...
    match serde_json::to_string(book) {
        Ok(s) => println!("{s}"),
        Err(e) => {
            eprintln!("mdbook-karn-visuals: failed to serialise book: {e}");
            exit(1);
        }
    }
}
