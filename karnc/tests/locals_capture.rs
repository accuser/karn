...
use karnc::locals::{LocalBinding, locals_at};
...
fn locals_for(result: &karnc::ProjectDiagnostics, file: &str) -> (Vec<LocalBinding>, String) {
    let text = result
        .files
        .iter()
...
fn every_binding_kind_is_recorded() {
    let result = karnc::diagnose_project(&fixture_root("inlay/clean"), &HashMap::new());
    let (locals, _) = locals_for(&result, "shop/util.karn");
...
fn nested_block_bindings_are_scoped_to_their_block() {
    let result = karnc::diagnose_project(&fixture_root("locals"), &HashMap::new());
    let (locals, text) = locals_for(&result, "m.karn");
