...
/// Wrap declarations in a minimal `commons` so `karnc::compile` can run them.
...
fn scalar_holes_emit_a_template_literal() {
    ...
    let ts = karnc::compile(&src, "demo.karn").expect("scalar holes type-check");
    assert!(
...
fn greet(who: Name) -> String { "Hi, \(who)!" }"#,
    ...
    let ts = karnc::compile(&src, "demo.karn").expect("a refined hole type-checks");
...
fn a_plain_string_stays_a_double_quoted_literal() {
    ...
    let ts = karnc::compile(&src, "demo.karn").unwrap();
...
fn a_non_scalar_hole_is_rejected() {
    ...
    let errs = karnc::compile(&src, "demo.karn").expect_err("a non-scalar hole is an error");
    assert!(
        errs.iter()
