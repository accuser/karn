...
//!
//! Each `docs/diagnostics/<id>.karn` is a deliberately *failing* Karn program.
...
//!     KARN_BLESS=1 cargo test -p karnc --test doc_diagnostics

use std::fs;
use std::path::PathBuf;

use karnc::CompileError;
...
fn compile_fixture(id: &str, source: &str) -> Result<(), Vec<CompileError>> {
    ...
    if first.starts_with("commons ") {
        karnc::compile(source, &format!("{id}.karn")).map(|_| ())
...
fn diagnostic_transcripts_are_up_to_date() {
    ...
    let bless = std::env::var_os("KARN_BLESS").is_some();
    ...
    for karn in fixtures {
        ...
        let transcript = karnc::render_errors_plain(&errors, &source, &format!("{id}.karn"));
        ...
        if current != transcript {
            failures.push(format!(
                "docs/diagnostics/{id}.txt is out of date with the compiler.\n\
                 Regenerate with: KARN_BLESS=1 cargo test -p karnc --test doc_diagnostics\n\
                 --- committed ---\n{current}\n--- current ---\n{transcript}"
            ));
        }
    }
