...
//! Skips loudly without a toolchain; `KARN_REQUIRE_TSC=1` turns the skip into a
//! failure.

use karnc::{BuildTarget, CompileOptions};
use std::fs;
use std::path::Path;
...
fn multi_actor_sum_resolves_first_wins_and_fails_closed() {
    ...
    let tmp = std::env::temp_dir().join(format!("karn-multi-actor-{}", std::process::id()));
    ...
    let out =
        match karnc::compile_project(&CompileOptions::single(&proj).target(BuildTarget::Workers)) {
            Ok(o) => o,
            Err(failure) => panic!(
                "compile the multi-actor sum project to Workers:\n{}",
                karnc::render_project_errors(&failure.flatten())
            ),
        };
