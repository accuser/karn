...
//!
//! `docs/src/reference/cli.md` is rendered from `karnc::cli` — the very same
//! command tree the binary parses with — so the page cannot describe a CLI the
//! binary does not have.
//!
//! Regenerate the docs page with:
//!     KARN_BLESS=1 cargo test -p karnc --test cli_reference

use std::fs;
use std::path::PathBuf;

use karnc::cli::{command, render_markdown};
...
fn every_subcommand_is_documented() {
    ...
    for sub in command().get_subcommands() {
        ...
        assert!(
            rendered.contains(&format!("`karnc {name}`")),
