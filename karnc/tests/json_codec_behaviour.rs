...
// 6) The Karn-side JsonError fields compose (describe() formats kind/path/message).
...
fn json_codec_behaviour() {
    ...
    let ts = karnc::compile(&source, "212_json_codec/input.karn")
        .expect("the json-codec fixture must compile");

    let tmp = std::env::temp_dir().join(format!("karn-json-codec-{}", std::process::id()));
