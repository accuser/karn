...
pub const REGISTRY: &[DiagnosticInfo] = &[
    ...
    d("karn.agents.bad_state_initialiser", "An agent state-field initialiser is not a static value of the field's type."),
    d("karn.agents.non_zeroable_state_field", "An agent state field has no initialiser and no implicit zero value."),
    ...
    d("karn.cron.bad_params", "An `on cron` handler declares more than one parameter, or a non-`Int` one."),
    ...
    d("karn.exports.capability_not_provided", "An exported capability has no provider in its context."),
    ...
    d("karn.exports.undeclared_capability", "`exports capability` names a capability that is not declared."),
    d("karn.exports.undeclared_type", "`exports` names a type that is not declared."),
    d("karn.given.cross_context_unknown_capability", "`given B.Cap` names a capability the consumed context does not export."),
    ...
    d("karn.parse.queue_in_agent", "An `on queue` handler was declared in an agent."),
    ...
    d("karn.queue.bad_params", "An `on queue` handler does not take exactly one `message` parameter."),
    d("karn.queue.duplicate_consumer", "Two `on queue` handlers consume the same queue."),
    d("karn.queue.invalid_name", "An `on queue` handler has an empty queue name."),
    d("karn.queue.return_not_effect_result", "An `on queue` handler does not return `Effect[Result[(), E]]`."),
    ...
    d("karn.types.is_base_mismatch", "An `is` refinement check is applied to a value of the wrong base type."),
